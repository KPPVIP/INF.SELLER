...
RegisterNetEvent("polo_infoillegal:notify")
AddEventHandler("polo_infoillegal:notify", function(icon, type, sender, title, text)
    Citizen.CreateThread(function()
		Wait(1)
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'info',
      {
        ...
        if data.current.value == 'weed' then
           TriggerServerEvent("polo_infoillegal:Weed")
		   ESX.UI.Menu.CloseAll()
        end
		...
		if data.current.value == 'opium' then
           TriggerServerEvent("polo_infoillegal:Opium")
		   ESX.UI.Menu.CloseAll()
        end
		...
		if data.current.value == 'coke' then
           TriggerServerEvent("polo_infoillegal:Coke")
		   ESX.UI.Menu.CloseAll()
        end
...
RegisterNetEvent("polo_infoillegal:WeedFarm")
AddEventHandler("polo_infoillegal:WeedFarm", function()
	if Config.GPS then
		x, y, z = Config.WeedFarm.x, Config.WeedFarm.y, Config.WeedFarm.z
		SetNewWaypoint(x, y, z)
		local source = GetPlayerServerId();
		TriggerEvent("polo_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('GPS'))
	else
		TriggerEvent("polo_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('RecWeed'))
	end
end)
...
RegisterNetEvent("polo_infoillegal:OpiumFarm")
AddEventHandler("polo_infoillegal:OpiumFarm", function()
	if Config.GPS then
		...
		TriggerEvent("polo_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('RecOpium'))
	end
end)
...
RegisterNetEvent("polo_infoillegal:CokeFarm")
AddEventHandler("polo_infoillegal:CokeFarm", function()
	if Config.GPS then
		...
		TriggerEvent("polo_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('RecCoke'))
	end
end)
...
AddEventHandler('polo_infoillegal:hasEnteredMarker', function(zone)
	CurrentAction     = 'menu_info_illegal'
	CurrentActionMsg  = _U('press_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('polo_infoillegal:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function ()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('polo_infoillegal:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('polo_infoillegal:hasExitedMarker', LastZone)
		end
	end
end)
				...
				if CurrentAction == 'menu_info_illegal' then
					if Config.Hours then
						if heure > Config.openHours and heure < Config.closeHours then	
							...
							TriggerServerEvent('polo_infoillegal:Nothere')
						end
					else
						OpenInfoIllegalMenu()
					end
